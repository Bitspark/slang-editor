...
export class SlangType {
	...
	public constructor(private parent: SlangType | null, private readonly typeIdentifier: TypeIdentifier, private readonly autoGenerated = false) {
	...
	public union(other: SlangType): SlangType {
		...
		if (this.typeIdentifier === TypeIdentifier.Map) {
			const newMap = new SlangType(this.parent, TypeIdentifier.Map, this.autoGenerated);
		...
		if (this.typeIdentifier === TypeIdentifier.Stream) {
			const newStream = new SlangType(this.parent, TypeIdentifier.Stream, this.autoGenerated);
	...
	public copy(): SlangType {
		const typeCopy = new SlangType(this.parent, this.typeIdentifier, this.autoGenerated);
		switch (this.typeIdentifier) {
			case TypeIdentifier.Map:
	...
	public anySubStreamManual(): boolean {
		if (this.typeIdentifier === TypeIdentifier.Map) {
			for (const sub of this.getMapSubs()) {
				if (sub[1].anySubStreamManual()) {
					return true;
				}
			}
		} else if (this.typeIdentifier === TypeIdentifier.Stream) {
			return this.getStreamSub().anySubStreamManual();
	...
	public getManualParts(): SlangType {
		...
		switch (this.typeIdentifier) {
			case TypeIdentifier.Map:
				for (const sub of this.getMapSubs()) {
					...
					const subType = sub[1].getManualParts();
					if (!subType.isVoid()) {
				...
				break;
			case TypeIdentifier.Stream:
				type.setStreamSub(this.getStreamSub().getManualParts());
				break;
			case TypeIdentifier.Generic:
				type.setGenericIdentifier(this.getGenericIdentifier());
				break;
			default:
				if (!this.anySubStreamManual()) {
